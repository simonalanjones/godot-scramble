...
func _ready():
	emit_signal('landscape_decoded')
	setup()
	
func setup():
	for stage in range(1, 7):
		var draw_runway = true if stage == 1 else false
		decode_landscape_stage(stage,draw_runway)
	#print(rocket_positions)
	emit_signal('landscape_decoded')
...
func restart(stage):
	...
	xpos = 0
	decode_landscape_stage(stage,true)
	#for draw_stage in range(stage+1, 7):
	#	decode_landscape_stage(draw_stage,false)
	emit_signal('landscape_draw_completed')
	
	

func decode_landscape_stage(stage:int, draw_runway:bool = false):
...
func add_ground_target(target_id,x_pos,y_pos):
	match target_id:
...
func get_data_for_stage(stage):
	match stage:
		...
		6: return LandscapeData.stage_6_landscape_layout
...
func get_rocket_positions():
	return rocket_positions


func get_mystery_positions():
	return mystery_positions
	
	
func get_fuel_positions():
