...
onready var new_stage = 6
onready var stage_data = LandscapeData.stage_6_landscape_layout
...
func get_data_from_stage():
	...
	update_landscape_pointer()
	return new_landscape_data
	
	
func decode_landscape_bottom():
	
	var first_y = (stage_data[new_pointer].hex_to_int() & 248) / 8
	var second_y = (stage_data[new_pointer + 2].hex_to_int() & 248) / 8
		
	var first_tile_id =  get_tile(stage_data[new_pointer + 1].hex_to_int())
	var second_tile_id = get_tile(stage_data[new_pointer + 3].hex_to_int())
	
	var ground_target_code
	if landscape_has_ceiling():
		ground_target_code = stage_data[new_pointer + 8].hex_to_int()
	else:
		ground_target_code = stage_data[new_pointer + 5].hex_to_int()
	
	var bottom_landscape_data:Array = []
	
	bottom_landscape_data.append({ 
			'ypos' : first_y,
			'outer_tile_id' : first_tile_id,
			'inner_tile_id' : 17 if new_stage > 3 else 14,
			'position' : 'bottom',
			#'ground_target_code' : ground_target_code,
			})
				
	bottom_landscape_data.append({ 
			'ypos' : second_y,
			'outer_tile_id' : second_tile_id,
			'inner_tile_id' : 34 if new_stage > 3 else 14,
			'position' : 'bottom',
			#'ground_target_code' : 0
...
func decode_landscape_top():
	var first_y = (stage_data[new_pointer + 4].hex_to_int() & 248) / 8
	var second_y = (stage_data[new_pointer + 6].hex_to_int() & 248) / 8

	var first_tile_id = get_tile(stage_data[new_pointer + 5].hex_to_int())	
	var second_tile_id = get_tile(stage_data[new_pointer + 7].hex_to_int())
...
func end_of_stage_reached() -> bool:
...
func update_landscape_pointer() -> void:
	if landscape_has_ceiling() == true:
		new_pointer += 9
	else:
		new_pointer += 6
...
func advance_landscape_stage() -> void:
	emit_signal('next_stage_reached', new_stage)
	new_stage += 1
	if new_stage > 6:
		new_stage = 6
...
func decode_landscape_stage(stage:int, draw_runway:bool = false)->void:
	...
	while not landscape[pointer].hex_to_int() == 255:
		...
		# y position bottom
		var first_y = (landscape[pointer].hex_to_int() & 248) / 8
		# y position bottom
